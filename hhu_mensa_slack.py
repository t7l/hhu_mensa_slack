...
# canteen (mensa) is open from monday to friday only and additionally closed on german holidays
...
SLACK_CHANNEL = '#mensa'  # set to an existing channel, where canteen menu will be posted
        ...
        if component_name:
            if component_name.lower() not in EXCLUDED_COMPONENTS:
                if ':' in component_name:
                    meal_components.append('_'+re.sub(r':','',component_name).strip()+'_')
                else:
...
def process_counter_prices(counter_description):
    ...
    counter_pricing = counter_description.find("ul", {"class":"price"}).find_all("li")
    counter_prices = {}

    for pricing in counter_pricing:
        pricing_details = pricing.find_all("span")
        if not len(pricing_details) == 2:
            print("Something is wrong with the pricing at counter {}.".format(counter_description))
        else:
...
def process_counter(counter):
    ...
    counter_description = counter.find("div", {"class":"counter-table"}).find("div", {"class":"description"})
    counter_meal = counter_description.find("ul", {"class":"menu"}).find_all("li")
    ...
    result = "{} {} (S: {}, M: {})".format(slack_format_string(counter_name+':', '*'), ", ".join(meal_components), counter_prices.get('Studierende'), counter_prices.get('Bedienstete'))
    return result
...
def scrape_and_format_canteen_menu(page_soup):
    ...
    todays_menu = page_soup.find("div", {"data-day":"{}".format(TODAY_DAYNAME)})
    ...
    output = '\n'.join(menus)
    output = slack_format_string('Heute ({}) in der Mensa:'.format(todays_date), '*') + '\n\n' + output
    return output
